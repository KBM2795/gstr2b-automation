  ...
  const handleFileSelect = async (type: "excel" | "storage") => {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).electronAPI) {
        ...
        if (type === "excel") {
          selectedPath = await (window as any).electronAPI.openFileDialog()
        } else {
          selectedPath = await (window as any).electronAPI.openDirectoryDialog()
        }
        ...
        // Fallback for web browser (development/testing)
        if (type === "excel") {
          ...
          input.onchange = (e) => {
            ...
            if (file) {
              // In browser, we can only get file name, not full path
              const mockPath = `C:\\Users\\Desktop\\${file.name}`
              setExcelPath(mockPath)
            }
          }
          input.click()
        } else {
          ...
          input.onchange = (e) => {
            ...
            if (files && files.length > 0) {
              ...
              const mockPath = `C:\\Users\\Desktop\\${folderName}`
              setStoragePath(mockPath)
            }
          }
          input.click()
        }
